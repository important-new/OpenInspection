...
describe('Presence Durable Objects — smoke', () => {
    ...
    // wrangler.toml (gitignored, generated by setup). A fresh clone has no
    // wrangler.toml until setup runs, but the structural check needs to
    // pass in CI / `npm test` without any setup prerequisite.
    it('wrangler.toml.example binds both classes', () => {
        const toml = readFileSync(resolve(__dirname, '../../wrangler.toml.example'), 'utf8');
