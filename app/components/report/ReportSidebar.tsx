...
export function ReportSidebar({ sections, role, inspectionId, brandLogo, companyName, onPublish }: ReportSidebarProps) {
  ...
  return (
    /* ds-allow: fixed-dark report sidebar (always dark in both themes) */
    <aside className="lg:fixed lg:top-0 lg:left-0 lg:bottom-0 lg:w-60 bg-slate-900 text-slate-200 flex flex-col z-30 print:hidden hidden lg:flex" aria-label="Report navigation">
      ...
      {role === "inspector" && (
        <div className="px-5 py-3 border-b border-slate-800 space-y-2">
          ...
          <button type="button" onClick={onPublish} className="block w-full h-9 px-3 rounded-md bg-ih-watch text-white text-[12px] font-bold hover:bg-ih-watch/85 transition-colors focus:outline-none focus:ring-2 focus:ring-white/30">
            Publish
          </button>
        </div>
      )}

      {/* ds-allow: fixed-dark report sidebar */}
        ...
        {sections.map((s) => {
          ...
          return (
            <a key={s.id} href={`#section-${s.id}`} onClick={() => setActiveSection(s.id)} className={`group flex items-center gap-2 px-5 py-2 text-[13px] font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition-colors focus:outline-none focus:bg-slate-800 focus:text-white ${isActive ? "bg-slate-800 text-white border-l-2 border-ih-primary -ml-0.5" : ""}`}>
              ...
              {total > 0 && (
                <span className="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1.5 rounded-full bg-ih-bad text-white text-[10px] font-bold tabular-nums" title={`${total} issue${total === 1 ? "" : "s"} in this section`} aria-label={`${total} issues`}>{total}</span>
              )}
            </a>
          );
        })}
      ...
      {/* ds-allow: fixed-dark report sidebar footer */}
      <div className="px-5 py-3 border-t border-slate-800 text-[11px] text-ih-fg-3 space-y-1">
        <div className="flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-ih-ok animate-pulse" aria-hidden="true" />
          <span>Live</span>
        </div>
      </div>
    </aside>
