...
export function ReportUnitsSummary({ units, defectCountsByUnit }: ReportUnitsSummaryProps) {
  ...
  return (
    <section className="max-w-3xl mx-auto px-4 mb-8">
      <div className="rounded-md border border-ih-border bg-ih-bg-card p-5 shadow-ih-card">
        <h3 className="text-[10px] font-bold uppercase tracking-widest text-ih-fg-4 mb-3">Property structure</h3>
        <ul className="space-y-3 text-sm">
          {buildings.map((building) => (
            <li key={building.id}>
              <a href={`#unit-${building.id}`} className="font-bold text-ih-fg-1 hover:text-ih-primary">{building.name}</a>
              <ul className="ml-3 mt-1 space-y-1">
                {childrenOf(units, building.id).map((floor) => (
                  <li key={floor.id}>
                    <a href={`#unit-${floor.id}`} className="font-medium text-ih-fg-2 hover:text-ih-primary">{floor.name}</a>
                    <ul className="ml-3 mt-1 space-y-1">
                      {childrenOf(units, floor.id).map((unit) => {
                        const count = counts[unit.id] ?? 0;
                        return (
                          <li key={unit.id} className="flex items-center gap-2">
                            <a href={`#unit-${unit.id}`} className="text-ih-fg-3 hover:text-ih-primary">{unit.name}</a>
                            {count > 0 && (
