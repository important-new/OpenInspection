    ...
    <div className="flex items-center gap-3 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-[14px] font-semibold text-ih-fg-1 truncate">{person.name}</span>
          ...
          {suffix && <span className="px-2 py-0.5 text-[10px] font-bold rounded-full whitespace-nowrap bg-ih-bg-muted text-ih-fg-2">{suffix}</span>}
        </div>
        <div className="text-[12px] mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5">
          {person.email && <a href={`mailto:${person.email}`} className="text-ih-primary hover:underline">{person.email}</a>}
          {person.phone && <a href={`tel:${person.phone}`} className="text-ih-primary hover:underline">{person.phone}</a>}
          {!person.email && !person.phone && <span className="text-ih-fg-4 italic">No contact info</span>}
        </div>
      </div>
    </div>
  );
...
export function PeopleCard({ inspector, client, buyerAgents = [], listingAgents = [] }: PeopleCardProps) {
  ...
  return (
    <section className="bg-ih-bg-card border border-ih-border rounded-md p-5">
      <header className="flex items-center justify-between mb-3">
        <h2 className="text-[14px] font-bold tracking-tight text-ih-fg-1">People</h2>
        <span className="text-[10px] font-mono text-ih-fg-4">{total} total</span>
      </header>
