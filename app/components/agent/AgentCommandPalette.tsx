...
export function AgentCommandPalette({ inspectors, agentSlug, bookingHost }: AgentCommandPaletteProps) {
  ...
  return (
    <>
        ...
        <div className="fixed inset-0 z-[10000] flex items-start justify-center pt-[12vh] px-4" role="dialog" aria-modal="true" aria-label="Agent command palette" data-testid="agent-command-palette">
          ...
          <div className="relative w-full max-w-[36rem] bg-ih-bg-card border border-ih-border rounded-2xl shadow-ih-popover flex flex-col overflow-hidden max-h-[70vh]">
            <div className="flex items-center gap-3 px-5 py-4 border-b border-ih-border">
              ...
              <span className="text-[11px] text-ih-fg-3 px-2 py-1 border border-ih-border rounded-md font-mono">Esc to close</span>
            </div>
            <div className="flex items-center gap-2.5 px-5 py-2.5 border-b border-ih-border">
              <svg className="w-[18px] h-[18px] text-ih-fg-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-4.35-4.35M16.5 10.5a6 6 0 11-12 0 6 6 0 0112 0z" /></svg>
              <input
                ref={inputRef}
                value={query}
                ...
                className="flex-1 border-0 outline-0 bg-transparent text-[15px] text-ih-fg-1 placeholder:text-ih-fg-4 py-1"
              />
            </div>
            <div className="flex-1 overflow-y-auto pb-2">
              {visible.length === 0 && <div className="py-10 text-center text-ih-fg-3 text-sm">No matches.</div>}
              {pages.length > 0 && (
                <div>
                  <div className="px-5 pt-3 pb-1.5 text-[11px] font-bold uppercase tracking-[0.18em] text-ih-fg-4">Pages</div>
                  {pages.map((item) => {
                    ...
                    return (
                      <button
                        key={item.id}
                        type="button"
                        className={`flex items-center gap-3 w-full px-5 py-2.5 text-left text-[15px] cursor-pointer transition-colors ${highlighted === vi ? "bg-ih-primary-tint text-ih-primary" : "text-ih-fg-1"}`}
                        onMouseEnter={() => setHighlighted(vi)}
                        onClick={() => activate(item)}
                        data-testid={`agent-cmdk-item-${item.id}`}
                      >
                        <span className="flex-1 min-w-0">{item.label}</span>
                        {item.hint && <span className="text-[11px] uppercase tracking-[0.12em] font-semibold text-ih-fg-4">{item.hint}</span>}
                      </button>
                    );
                  })}
                </div>
              ...
              {actions.length > 0 && (
                <div>
                  <div className="px-5 pt-3 pb-1.5 text-[11px] font-bold uppercase tracking-[0.18em] text-ih-fg-4">Actions</div>
                  {actions.map((item) => {
                    const vi = visible.indexOf(item);
                    return (
                      <button
            ...
            <div className="flex items-center justify-between px-5 py-2.5 border-t border-ih-border bg-ih-bg-muted text-[11px] text-ih-fg-3 tracking-wide">
              <span><kbd className="bg-ih-bg-card border border-ih-border rounded px-1.5 py-0.5 font-mono text-[10px] text-ih-fg-1 mx-0.5">&uarr;</kbd><kbd className="bg-ih-bg-card border border-ih-border rounded px-1.5 py-0.5 font-mono text-[10px] text-ih-fg-1 mx-0.5">&darr;</kbd> navigate &middot; <kbd className="bg-ih-bg-card border border-ih-border rounded px-1.5 py-0.5 font-mono text-[10px] text-ih-fg-1 mx-0.5">&crarr;</kbd> open &middot; <kbd className="bg-ih-bg-card border border-ih-border rounded px-1.5 py-0.5 font-mono text-[10px] text-ih-fg-1 mx-0.5">&amp;#8984;K</kbd> toggle</span>
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-ih-bg-inverse text-ih-fg-inverse px-4 py-2.5 rounded-full text-[13px] shadow-ih-popover z-[10001]">
          {toast}
        </div>
      )}
    </>
