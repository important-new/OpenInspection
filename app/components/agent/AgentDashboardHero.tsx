...
export function AgentDashboardHero({
  ...
  return (
    /* ds-allow: fixed-dark agent dashboard hero panel (always dark in both themes) */
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <div className="space-y-2 min-w-0 flex-1">
            ...
            {inspectorName && (
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                {/* ds-allow: glass overlay on the fixed-dark hero */}
                <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-[11px] font-bold uppercase tracking-wider">
                  <span className="w-1.5 h-1.5 rounded-full bg-ih-ok" aria-hidden="true" />
          ...
          <div className="flex flex-col sm:flex-row md:flex-col gap-2 flex-shrink-0">
            <button
              ...
              className="h-10 px-5 rounded-md bg-ih-agent-accent text-ih-agent-accent-fg text-[13px] font-bold inline-flex items-center justify-center gap-1.5 hover:bg-ih-agent-accent/85 active:scale-95 transition-all focus:outline-none focus:ring-2 focus:ring-white/30 shadow-ih-popover"
            >
