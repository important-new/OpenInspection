...
describe("SideRail — Library tab type gate (module E)", () => {
  it("fill mode: shows Library for a rich item", () => {
    render(<SideRail locale="en-US" mode="fill" initialOpen activeItem={{ id: "i1", label: "Attic", type: "rich" }} />);
    expect(screen.getByRole("button", { name: /Library/ })).toBeTruthy();
  });

  it("fill mode: hides Library for a non-rich item", () => {
    render(<SideRail locale="en-US" mode="fill" initialOpen activeItem={{ id: "i2", label: "Year", type: "number" }} />);
  ...
  it("author mode: always shows Library regardless of item type", () => {
    render(<SideRail locale="en-US" mode="author" initialOpen activeItem={{ id: "i3", label: "Year", type: "number" }} />);
    expect(screen.getByRole("button", { name: /Library/ })).toBeTruthy();
  });
});
