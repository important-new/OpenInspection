...
export function InspectorToolsDock({
  ...
  return (
    <div className="hidden md:block fixed bottom-6 right-6 z-40">
      ...
      {dockOpen && (
        <div className="absolute bottom-16 right-0 mb-2 ih-card p-2 min-w-[200px] bg-ih-bg-card border border-ih-border rounded-lg shadow-ih-popover" role="menu" aria-label={m.editor_dock_menu_label()}>
          {TILES.map((t) => (
            <MenuItem
              key={t.id}
              icon={
      ...
      <button
        ...
        aria-label={dockOpen ? m.editor_dock_close() : m.editor_dock_open()}
