 ...
 *
 * Every createRoute() call in src/api/ must wrap its config with
 * withMcpMetadata(...) so the OpenAPI doc carries `x-scopes` and `x-tier`
 * vendor extensions. The route-metadata vitest gate (src/__tests__/route-
 * metadata.test.ts) enforces this on CI.
 */
