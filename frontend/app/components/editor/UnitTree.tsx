  ...
  return (
    <aside className="w-56 border-r border-ih-border bg-ih-bg-app p-3 overflow-y-auto" aria-label="Unit hierarchy">
      <div className="flex items-center justify-between mb-2">
        ...
        <button
          className="px-2 h-6 rounded-md bg-ih-bg-card border border-ih-border text-xs font-bold hover:bg-slate-100 dark:hover:bg-slate-700"
        ...
        {roots.map((b) => (
          <li key={b.id}>
            <div className={`flex items-center gap-1 ${selectedUnitId === b.id ? "bg-indigo-100 dark:bg-indigo-900/30 rounded" : ""}`}>
              ...
              <button className="text-slate-400 hover:text-ih-primary px-1" onClick={() => onAddUnit?.(b.id, "floor")} title="Add floor">+</button>
            </div>
              ...
              {childrenOf(b.id).map((f) => (
                <li key={f.id}>
                  <div className={`flex items-center gap-1 ${selectedUnitId === f.id ? "bg-indigo-100 dark:bg-indigo-900/30 rounded" : ""}`}>
                    ...
                    <button className="text-slate-400 hover:text-ih-primary px-1" onClick={() => onAddUnit?.(f.id, "unit")} title="Add unit">+</button>
                  </div>
                  <ul className="ml-3 mt-1 space-y-1">
                    {childrenOf(f.id).map((u) => (
                      <li key={u.id}>
                        <button
                          className={`text-left px-2 py-1 w-full text-ih-fg-3 ${selectedUnitId === u.id ? "bg-indigo-100 dark:bg-indigo-900/30 rounded" : ""}`}
