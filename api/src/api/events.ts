...
export const eventsRoutes = createApiRouter()
    .get('/event-types', requireRole(['owner', 'admin', 'inspector']), async (c) => {
        const data = await c.var.services.event.listEventTypes(c.get('tenantId'));
        return c.json({ success: true, data });
    })
    .post('/event-types', requireRole(['owner', 'admin']), async (c) => {
    ...
    .put('/event-types/:id', requireRole(['owner', 'admin']), async (c) => {
    ...
    .delete('/event-types/:id', requireRole(['owner', 'admin']), async (c) => {
        const id = c.req.param('id') as string;
        await c.var.services.event.deactivateEventType(c.get('tenantId'), id);
        return c.json({ success: true });
    })
    .post('/event-types/seed', requireRole(['owner', 'admin']), async (c) => {
        const r = await c.var.services.event.bulkSeed(c.get('tenantId'));
        return c.json({ success: true, data: r });
    })
    // ---- Inspection events ----
    .get('/inspections/:inspectionId/events', requireRole(['owner', 'admin', 'inspector']), async (c) => {
    ...
    .post('/inspections/:inspectionId/events', requireRole(['owner', 'admin', 'inspector']), async (c) => {
        const inspectionId = c.req.param('inspectionId') as string;
    ...
    .put('/events/:id', requireRole(['owner', 'admin', 'inspector']), async (c) => {
        const id = c.req.param('id') as string;
    ...
    .delete('/events/:id', requireRole(['owner', 'admin']), async (c) => {
        const id = c.req.param('id') as string;
        await c.var.services.event.deleteEvent(c.get('tenantId'), id);
        return c.json({ success: true });
    })
    .get('/events/upcoming', requireRole(['owner', 'admin', 'inspector']), async (c) => {
...
export type EventsApi = typeof eventsRoutes;
