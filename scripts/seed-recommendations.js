#!/usr/bin/env node
/**
 * Seeds 80 pre-written inspection recommendations into the recommendations table.
 * Run: npm run seed:recommendations
 *      TENANT_ID=<uuid> node scripts/seed-recommendations.js          # standalone prod
 *      TENANT_ID=<uuid> node scripts/seed-recommendations.js --local  # local dev
 *      TENANT_ID=<uuid> node scripts/seed-recommendations.js --saas   # saas prod (uses wrangler.saas.toml)
 *
...
const countResult = execSync(
    `npx wrangler d1 execute ${DB_NAME} ${flag} ${configFlag} --json --command "SELECT COUNT(*) as c FROM recommendations WHERE tenant_id = '${TENANT_ID}'"`,
    { encoding: 'utf8' },
);
let existingCount = 0;
try {
    ...
    if (jsonStart >= 0) {
        existingCount = JSON.parse(countResult.slice(jsonStart))?.[0]?.results?.[0]?.c ?? 0;
    }
} catch { /* fallback to 0 — seed will proceed */ }

if (existingCount >= SEEDS.length) {
    console.log(`Seed skipped: tenant '${TENANT_ID}' already has ${existingCount} recommendations (>= ${SEEDS.length}).`);
    process.exit(0);
}

// ── Build SQL ──────────────────────────────────────────────────────────────────
// created_at is INTEGER (Unix seconds) per migration 0026_recommendations.sql
const nowSec = Math.floor(Date.now() / 1000);

const values = SEEDS.map(r => {
    const id  = randomUUID();
    const cat = r.category.replace(/'/g, "''");
    const nm  = r.name.replace(/'/g, "''");
    const sum = r.defaultRepairSummary.replace(/'/g, "''");
    const eMin = r.defaultEstimateMin === null ? 'NULL' : r.defaultEstimateMin;
    const eMax = r.defaultEstimateMax === null ? 'NULL' : r.defaultEstimateMax;
    return `('${id}', '${TENANT_ID}', '${cat}', '${nm}', '${r.severity}', ${eMin}, ${eMax}, '${sum}', NULL, ${nowSec})`;
}).join(',\n');

const sql = `INSERT INTO recommendations (id, tenant_id, category, name, severity, default_estimate_min, default_estimate_max, default_repair_summary, created_by_user_id, created_at) VALUES\n${values};`;

// ── Write temp file and execute ────────────────────────────────────────────────
const tmpDir  = join(tmpdir(), 'oi-seed-recommendations');
mkdirSync(tmpDir, { recursive: true });
const sqlFile = join(tmpDir, 'recommendations.sql');
...
try {
    ...
    console.log(`Seeded ${SEEDS.length} recommendations for tenant '${TENANT_ID}'.`);
} catch (e) {
